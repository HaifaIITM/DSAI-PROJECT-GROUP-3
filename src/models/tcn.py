...
class CausalConv1d(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, dilation=1):
        super().__init__()
        pad = (kernel_size - 1) * dilation  # left-pad for causality
...
class _TCNBlock(nn.Module):
    def __init__(self, ch_in, ch_out, kernel_size=3, dilation=1, dropout=0.0):
        super().__init__()
        self.net = nn.Sequential(
            CausalConv1d(ch_in, ch_out, kernel_size, dilation),
            nn.ReLU(),
            nn.Dropout(dropout),
            CausalConv1d(ch_out, ch_out, kernel_size, dilation),
            nn.ReLU(),
            nn.Dropout(dropout),
        )
...
class _TCNHead(nn.Module):
    def __init__(self, in_dim, channels=(64, 64), kernel_size=3, dropout=0.0, out_dim=1):
        ...
        for i, ch in enumerate(channels):
            dilation = 2 ** i
            layers.append(_TCNBlock(ch_in, ch, kernel_size=kernel_size, dilation=dilation, dropout=dropout))
            ch_in = ch
        self.tcn = nn.Sequential(*layers)
        self.head = nn.Linear(ch_in, out_dim)
...
class TCNRegressor:
    """
    Temporal ConvNet baseline with causal dilated 1D convolutions and residual blocks.
    """
    def __init__(self,
                 seq_len: int = 32,
